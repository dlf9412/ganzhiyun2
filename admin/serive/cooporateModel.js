...
router.post(httpUrl.cooporateModelAdd, (req, res) => { //添加
  console.log(req.body)
  req.body.data = new Date().format("yyyy-MM-dd hh:mm:ss")
  add(model.cooperatemodel, req.body).then((response) => {
    res.send(response)
  })
})

router.post(httpUrl.cooporateModelSelect, (req, res) => { //分页和条件查询
  console.log(req.body)
  // req.body.data = new Date().format("yyyy-MM-dd hh:mm:ss")
  select(model.cooperatemodel, req.body).then((response) => {
    res.send(response)
  })
})


router.post(httpUrl.cooporateModelUpdata, (req, res) => { //更新
  req.body.data = new Date().format("yyyy-MM-dd hh:mm:ss")
  updata(model.cooperatemodel, {
    _id: req.body._id
...
router.post(httpUrl.cooporateModelDelete, (req, res) => { //删除
  delete1(model.cooperatemodel, req.body).then((response) => {
    res.send(response)
  })
  // console.log(req.body)
  // let obj = [];
