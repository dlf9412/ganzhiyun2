...
router.post(httpUrl.cooporateAdd, (req, res) => { //添加新闻
  console.log(req.body)
  req.body.data = new Date().format("yyyy-MM-dd hh:mm:ss")
  add(model.cooporate, req.body).then((response) => {
    res.send(response)
  })
})

router.post(httpUrl.cooporateSelect, (req, res) => { //分页和条件查询新闻
  console.log(req.body)
  // req.body.data = new Date().format("yyyy-MM-dd hh:mm:ss")
  select(model.cooporate, req.body).then((response) => {
    res.send(response)
  })
})


router.post(httpUrl.cooporateUpdata, (req, res) => {
  req.body.data = new Date().format("yyyy-MM-dd hh:mm:ss")
  updata(model.cooporate, {
    _id: req.body._id
